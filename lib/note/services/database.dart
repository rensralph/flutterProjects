...
class NotesDatabaseService {
  ...
  init() async {
    ...
    return await openDatabase(path, version: 1,
      ...
      await db.execute(
          'CREATE TABLE Notes ( _id INTEGER PRIMARY KEY, title TEXT, content TEXT, isImportant INTEGER, date TEXT;');
  ...
  Future<List<NotesModel>> getNotesFromDB() async {
    ...
    List<Map> maps = await db.query('Notes',
        columns: ['_id', 'title', 'content', 'isImportant', 'date']);
    if (maps.length > 0) {
      maps.forEach((map) {
  ...
  Future<NotesModel> addNoteInDB(NotesModel newNote) async {
    ...
    int id = await db.transaction((transaction) => transaction.rawInsert(
          'INSERT into Notes (title, content, isImportant, date ) VALUES ("${newNote.title}", "${newNote.content}", "${newNote.isImportant == true ? 1 : 0}", "${newNote.date.toIso8601String()}");'));
    newNote.id = id;
